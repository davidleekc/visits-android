...
//                add(createStatusMarker(Status.INACTIVE))
    ...
    val MOCK_TRIPS_JSON: String by lazy {
        MyApplication.context.resources.openRawResource(R.raw.mock_trips_json).bufferedReader()
            .use { it.readText() }
    }

    val MOCK_TRIP_V1_JSON: String by lazy {
        MyApplication.context.resources.openRawResource(R.raw.mock_trip_v1_json).bufferedReader()
            .use { it.readText() }
    }
}